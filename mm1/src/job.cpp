...
void Job::loadManualOverrideSettings() {

	Glib::RefPtr<Gio::File> file = Gio::File::create_for_path("/etc/init/" + name + ".override");
	if (file->query_exists()) {
		// Ok file exists, we search for "manual" line and "Generated by JobMan" comment line
		Glib::RefPtr<Gio::FileInputStream> inStream = file->read();
		Glib::RefPtr<Gio::DataInputStream> dis = Gio::DataInputStream::create(inStream);

		std::string line;

		while (dis->read_line(line)) {
			if (line.find("manual") != string::npos) {
				manual = true;
			}
			if (line.find(GENERATED_BY_JOB_MAN_COMMENT) != string::npos) {
				canBeSetToManual = true;
			}
		}
	} else {
		// File does not exists, we can set it to "manual" if needed
		manual = false;
		canBeSetToManual = true;
	}
}
